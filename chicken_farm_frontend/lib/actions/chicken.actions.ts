...
//if running the frontend in Docker, use the internal Docker network address to reach the backend
const BACKEND_URL =
  process.env.CHICKEN_BACKEND_API_URL || 'http://host.docker.internal:4000/api/chickens';
...
export async function addChicken(data: Omit<Chicken, 'id' | 'createdAt'>): Promise<Chicken> {
